...
//? Массив объектов превращаем в объекст объектов
  ...
  function createLi({_id, title, body, completed = false} = {}) {
  ...
  function createTask(title, body){
    ...
    pushToLocal(NewTask._id,NewTask);
  ...
  function activateShowButton(e = 0){
    ...
    if(!e && !showCom.classList.contains(`active`) && !showAll.classList.contains(`active`)){
  ...
  //todo Пушим из локал стор в массив
  function localPush(){
      if (!Object.keys(localStorage).length) return;
      Object.keys(localStorage).forEach(key =>{
        
        arrOfTasks.push(JSON.parse(localStorage[key]));
  ...
  //todo Пушим в локал
  function pushToLocal(id,obj = objOfTasks[id]){
    try {
